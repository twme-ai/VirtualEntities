...
cp "${PROJECT_DIR}/build/libs/VirtualEntitiesIntegration.jar" \
  "${SERVER_DIR}/plugins/VirtualEntitiesIntegration.jar"
...
printf '%s\n' \
  ...
  'motd=VirtualEntities integration test' \
...
for _ in $(seq 1 180); do
  ...
  if ! kill -0 "${SERVER_PID}" 2>/dev/null; then
    tail -120 "${SERVER_DIR}/server.log" >&2
    exit 1
  fi
  sleep 1
done
...
if ! VE_E2E_PORT="${PORT}" node "${PROJECT_DIR}/integration/mineflayer/test.mjs"; then
